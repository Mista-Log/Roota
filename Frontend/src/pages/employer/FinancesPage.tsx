...
export default function EmployerFinancesPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const transData = await apiGet('/api/employer/finances/transactions/');
        setRecentTransactions(Array.isArray(transData.results) ? transData.results : transData);
      } catch (error) {
        console.warn('Error fetching employer finances data, using fallback:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleFundsSubmit = async (payload: {
    ...
    try {
      ...
      console.warn('Employer funds action failed:', error);
    }
  };
