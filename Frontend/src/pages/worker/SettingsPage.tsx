...
export default function WorkerSettingsPage() {
  ...
  const handleBillingAction = (key: string) => {
    ...
    link.href = url;
    link.download = 'roota-worker-invoices.csv';
    link.click();
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 rounded-2xl border border-border bg-card p-8 shadow-sm">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <motion.button whileHover={{ y: -1 }} onClick={() => fileInputRef.current?.click()} className="mt-3 rounded-lg border border-border bg-slate-50 px-4 py-2 text-sm font-medium transition-colors hover:bg-slate-100">Upload Photo</motion.button>
                <input ref={fileInputRef} type="file" accept=".png,.jpg,.jpeg" className="hidden" onChange={(e) => setUploadedPhotoName(e.target.files?.[0]?.name ?? null)} />
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              ...
              <div><label className="mb-2 block text-sm font-semibold text-slate-900">Bio</label><input type="text" value={formData.bio} onChange={(e) => setFormData({ ...formData, bio: e.target.value })} className="w-full rounded-xl border border-border bg-white px-4 py-2.5 text-sm outline-none transition-colors focus:border-primary-dark" /></div>
            </div>
            <motion.button whileHover={{ y: -2 }} onClick={handleSave} className="inline-flex items-center gap-2 rounded-xl bg-primary-dark px-5 py-3 text-sm font-semibold text-white hover:bg-primary-dark/90"><Save size={16} />{saving ? 'Saving...' : 'Save changes'}</motion.button>
          </motion.div>
        </Section>
      )}
