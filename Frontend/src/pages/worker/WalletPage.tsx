    ...
    <div className="space-y-6">
          ...
          <div className="flex items-start justify-between gap-6">
            <div className="max-w-md">
              ...
              <div className="mt-8 grid max-w-[370px] grid-cols-2 gap-3">
                <div className="rounded-2xl bg-white/16 px-4 py-3 backdrop-blur-[1px]"><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-white/75">Monthly Yield</p><p className="mt-1 text-xl font-bold">+4.2%</p></div>
                <div className="rounded-2xl bg-white/16 px-4 py-3 backdrop-blur-[1px]"><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-white/75">Locked Rewards</p><p className="mt-1 text-xl font-bold">$1,200.00</p></div>
              </div>
              <div className="mt-10 text-sm font-semibold tracking-wide text-white/90">Account Holder: Worker</div>
            </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.35fr_0.9fr]">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-2xl border border-border bg-card px-6 py-6 shadow-sm">
          <div className="mb-6"><h3 className="text-lg font-semibold text-slate-900">Monthly Earnings</h3><p className="text-sm text-muted">Payroll history for the last 6 months</p></div>
          <div className="h-64"><ResponsiveContainer width="100%" height="100%"><BarChart data={earningsData}><CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" vertical={false} /><XAxis dataKey="month" stroke="#9CA3AF" style={{ fontSize: '12px' }} /><YAxis stroke="#9CA3AF" style={{ fontSize: '12px' }} /><Tooltip contentStyle={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '8px' }} cursor={{ fill: 'rgba(11, 93, 75, 0.05)' }} /><Bar dataKey="value" fill="#032F2A" radius={[8, 8, 0, 0]} /></BarChart></ResponsiveContainer></div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="rounded-2xl border border-border bg-card px-6 py-6 shadow-sm">
          <div className="mb-6 flex items-center justify-between"><div><h3 className="text-lg font-semibold text-slate-900">Recent Transactions</h3></div><button onClick={() => navigate('/worker/transactions')} className="text-xs font-semibold text-primary-dark hover:text-primary">View All</button></div>
